...
int prims()
{
	...
	*/
	...
	#pragma omp single
	{
		...
		ndata[randNode][1]=1;
		/*The last thread will check the random node.The rest of the threads are not needed right now.*/	
	}
	while(!finish)
	{
			...
			for( tempCounter2=0; tempCounter2<numNode; tempCounter2++ )
			{
				...
				*/
				if( (graphTable[tempCounter1][tempCounter2] > 0) && (graphTable[tempCounter1][tempCounter2] < ndata[tempCounter1][2]) && (ndata[tempCounter2][1]==0))
				{
					...
					ndata[tempCounter1][2] = graphTable[tempCounter1][tempCounter2];
					ndata[tempCounter1][3] = tempCounter2;
					ndata[tempCounter1][4] = tempCounter1;
				}
			}
		}
